...
//        exit;
//      }
//      $fe = Configuration::getFrontendConfiguration();
      $configurationRepository = GeneralUtility::makeInstance('Causal\\IgLdapSsoAuth\\Domain\\Repository\\ConfigurationRepository');
      $configuration = $configurationRepository->findAll();
      if( count( $configuration ) == 1 ){
        $configuration = $configuration[0];
      ...
      $this->config = [
        'issuer' => Configuration::getValue("ADFSIssuer"),
        'clientId' => "service://" . Configuration::getValue("ADFSClientId"),
        'ressourceId' => "service://" . Configuration::getValue("ADFSClientId"),
        'clientSecret' => Configuration::getValue("ADFSClientSecret"),
        'redirectUri' => Configuration::getValue("ADFSRedirectUri"),
        'scope' => array(
//          "allatclaims", "profile", "email"
...
//            "scope" => array("allatclaims", "profile", "email")
          ),
        )
      ];
    }
      ...
      $url = $tempUrl['path'] . (!empty($tempUrl['query']) ? '?' . $tempUrl['query'] : '');
        ...
        $authorizationUrl = $this->provider->getAuthorizationUrl(
            [
                ...
                'resource' => $this->config['ressourceId'] ?? $this->config['clientId']
            ]
        );
